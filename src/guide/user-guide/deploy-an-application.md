...
K3s安装完成后，需要增加入方向的`6443`端口。详情参考 [安全组规则](https://help.aliyun.com/document_detail/25471.htm?spm=a2c4g.353191.0.0.557a235djwgvC9#concept-sm5-2wz-xdb)。
...
1. 下载 [命令行工具](https://github.com/nautes-labs/cli/releases/tag/v0.2.0)，执行以下命令，以准备运行环境 。
```Shell
...
1. 修改本地代码库中 Ingress 资源的域名：/deployment/test/devops-sample-svc.yaml
```yaml
...
spec:
  ...
  - host: devops-sample.$cluster-ip.nip.io
    http:
      ...
      ...
```
1. 访问 [GitLab](installation.md#查看安装结果)，设置 GitLab 账号有向 main 分支强制推送代码的权限，详情参考[保护分支启用强制推送](https://docs.gitlab.com/ee/user/project/protected_branches.html#allow-force-push-on-a-protected-branch)。

2. 推送 Kubernetes 资源清单至产品的代码库。
```Shell
